...
// Evaluation -----------------------------------
  ...
  def eval1(x: Data, env: Environment): Data = x match {
    ...
    case 'def :: (Symbol(name) :: params) :: body :: expr :: Nil =>
      eval('def :: Symbol(name) :: ('lambda :: params :: body :: Nil) :: expr :: Nil, env)
      
    case 'case :: expr_scrut :: conditions => 
      conditions match {
        case List(value, expr):: Nil => expr
        case List(value, expr):: tail => if (eval(expr_scrut, env) == value) expr else eval('case :: expr_scrut :: tail, env)
      }
