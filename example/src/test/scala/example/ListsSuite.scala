...
class ListsSuite extends FunSuite {
  ...
  test("sum returns 0 if the list is empty") {
    assert(sum(List()) === 0)
  }
  
  test("sum of a few numbers with negatives") {
    assert(sum(List(4,5,-3)) === 6)
  }
  ...
  test("max throws an exeption if the list is empty") {
  ...
  test("max of a few numbers with negatives") {
    assert(max(List(-6,-3,-2)) === -2)
  }
  
  test("max of a few number with repeated elements") {
    assert(max(List(6,7,7)) === 7)
  }
}