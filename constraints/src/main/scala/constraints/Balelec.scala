...
object Balelec {
  ...
  def schedule(
    ...
    val varsMatrix: Map[(Task, Volunteer), PropVar] =
      tasks.flatMap({ case t@Task(name, capacity) =>
        volunteers.map(v => (t, v) -> propVar(name))
      }).toMap
  
    val doNotGetNonDesirableTask: Seq[Formula] = (for (volunteer <- volunteers) 
        yield varsMatrix.filter(p => p._1._2 == volunteer).filter(
            i => !availability.get(i._1._2).get.contains(i._1._1)).foldLeft[Formula](true)(
                (acc, next) => acc && !next._2)).toSeq 
  
    val doNotGetTooMuchTasks: Seq[Formula] = (for (volunteer <- volunteers)
        yield positivesLessEquals(varsMatrix.filter(p => p._1._2 == volunteer).toList.map(j => j._2), maxWorkload)).toSeq  

    val doNotAssignTooFewVolunteers: Seq[Formula] = tasks.flatMap {
      case t@Task(name, capacity) =>
        val vols = volunteers.map { v => varsMatrix(t, v) }  
        val (binaryCount, additionalConstraints) = countPositives(vols)
        additionalConstraints + lessEqualsConst(capacity, binaryCount)
    }
        
    val allConstraints: Seq[Formula] = 
      doNotGetNonDesirableTask ++ doNotGetTooMuchTasks ++ doNotAssignTooFewVolunteers
  
    val res = solveForSatisfiability(and(allConstraints:_*))
  
    res.map(model => {
      tasks.map(task => {
        val assignedVolunteer = volunteers.filter(volunteer => model(varsMatrix((task, volunteer))))
        (task, assignedVolunteer.toList)
      }).toMap
    })
